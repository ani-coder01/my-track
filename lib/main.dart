...
class Expense {
  ...
  Expense copyWith({String? tag}) =>
      Expense(id: id, name: name, amount: amount, frequency: frequency, tag: tag ?? this.tag);
}
...
const _demoExpenses = [
  Expense(id: '1', name: 'Swiggy dinners',    amount: 350,  frequency: 'weekly',  tag: 'impulse'),
  Expense(id: '2', name: 'Netflix + OTT',     amount: 1299, frequency: 'monthly', tag: 'avoidable'),
  Expense(id: '3', name: 'Metro commute',     amount: 55,   frequency: 'daily',   tag: 'essential'),
  Expense(id: '4', name: 'Weekend cafe runs', amount: 900,  frequency: 'weekly',  tag: 'avoidable'),
  Expense(id: '5', name: 'Cloud storage',     amount: 299,  frequency: 'monthly', tag: 'avoidable'),
  Expense(id: '6', name: 'Gym membership',    amount: 1499, frequency: 'monthly', tag: 'essential'),
];

const _demoGoals = [
  Goal(id: 'g1', name: 'Trip to Japan',  targetAmount: 250000, targetDate: '2027-09-01', priority: 2, savedAmount: 72000),
  Goal(id: 'g2', name: 'Emergency fund', targetAmount: 400000, targetDate: '2026-12-01', priority: 1, savedAmount: 168000),
];
...
class AppState extends ChangeNotifier {
  List<Expense> expenses = List.from(_demoExpenses);
  List<Goal>    goals    = List.from(_demoGoals);
  double monthlySalary  = 85000;
  double sipAmount      = 12000;
  double sipReturn      = 12;
  int    sipMonths      = 120;
  ...
  void toggleExpenseTag(String id) {
    expenses = expenses.map((e) {
      if (e.id != id) return e;
      final next = e.tag == 'essential' ? 'avoidable' : e.tag == 'avoidable' ? 'impulse' : 'essential';
...
class AppStateProvider extends InheritedNotifier<AppState> {
  const AppStateProvider({super.key, required AppState state, required super.child})
...
class _AppShellState extends State<AppShell> {
  ...
  Future<void> _onAppOpen(String pkg, String appName) async {
    ...
    await Navigator.of(context).push(PageRouteBuilder(
      ...
      pageBuilder: (_, __, ___) => NudgeScreen(
        packageName: pkg,
        appName:     appName,
        riskScore:   risk,
      ),
...
class Pill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    switch (tone) {
      case 'teal':
        bg = kTeal.withOpacity(0.12); fg = kTeal;
      case 'positive':
        bg = kGreen.withOpacity(0.12); fg = kGreen;
      case 'warning':
        bg = kAmber.withOpacity(0.12); fg = kAmber;
      default:
        bg = Colors.white.withOpacity(0.06); fg = kMuted;
    }
    return Container(
      ...
      child: Text(label, style: jakarta(size: 11, weight: FontWeight.w700, color: fg)),
    );
  }
}
...
class Eyebrow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(
        text.toUpperCase(),
        style: jakarta(size: 11, weight: FontWeight.w800, color: kTeal, letterSpacing: 2),
      );
}
...
class SectionHeader extends StatelessWidget {
  ...
  final String title;
  const SectionHeader({super.key, this.eyebrow, required this.title, this.description});

  @override
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      if (eyebrow != null) ...[Eyebrow(eyebrow!), const SizedBox(height: 8)],
      Text(title, style: spaceGrotesk(size: 24, weight: FontWeight.w700, letterSpacing: -0.8)),
      if (description != null) ...[
    ...
    return Row(
      children: items.map((item) => Expanded(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 2),
          Text(item.value, style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
        ]),
      )).toList(),
    );
  }
...
class ProgressBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final col = tone == 'positive' ? kTeal : tone == 'warning' ? kAmber : kRed;
    return Container(
      height: 6,
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(99),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            gradient: const LinearGradient(colors: [kTeal, Color(0x66_35F0D2)]),
            boxShadow: [BoxShadow(color: kTeal.withOpacity(0.8), blurRadius: 12)],
          ),
        ),
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return PageFrame(children: [
      ...
      Row(children: [
        Expanded(child: _StatBox(label: 'Salary',    value: _fmtINR(state.monthlySalary), color: kGreen)),
        const SizedBox(width: 10),
        Expanded(child: _StatBox(label: 'Spent',     value: _fmtINR(total),               color: kRed)),
        const SizedBox(width: 10),
        Expanded(child: _StatBox(label: 'Saved',     value: '${saveRate.toStringAsFixed(0)}%', color: kTeal)),
      ]),
      const SizedBox(height: 12),
      ...
      GlassCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Income vs Expenses', style: spaceGrotesk(size: 15, weight: FontWeight.w700)),
          const SizedBox(height: 4),
          Text('6 months · all figures monthly', style: jakarta(size: 12, color: kMuted)),
          const SizedBox(height: 16),
          _MiniLineChart(
      ...
      GlassCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Where it goes', style: spaceGrotesk(size: 15, weight: FontWeight.w700)),
    ...
    return GlassCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text('MONEY LEAKING RIGHT NOW',
              style: jakarta(size: 10, color: kMuted, weight: FontWeight.w600, letterSpacing: 1)),
        ]),
        const SizedBox(height: 10),
        Text.rich(TextSpan(children: [
          TextSpan(text: _fmtINR(perSec),
              style: spaceGrotesk(size: 28, weight: FontWeight.w700, letterSpacing: -1)),
          TextSpan(text: '/sec', style: jakarta(size: 13, color: kMuted)),
        ])),
        const SizedBox(height: 4),
        Text('${_fmtINR(goneSince)} gone since you opened this page',
            style: jakarta(size: 12, color: kMuted)),
        const Divider(color: Color(0x10FFFFFF), height: 24),
        Row(children: [
          _BleedMeta(label: 'Monthly leakage', value: _fmtINR(monthly), color: kText),
          const SizedBox(width: 24),
          _BleedMeta(label: '10-year cost', value: _fmtINR(monthly * 120), color: kAmber),
        ]),
      ]),
    );
  }
  ...
  Widget _buildScoreCard(int score) {
    final label = score >= 75 ? 'Excellent' : score >= 50 ? 'Good' : score >= 30 ? 'Fair' : 'Poor';
    return GlassCard(
      child: Row(children: [
        // Ring
        SizedBox(
          ...
          child: CustomPaint(
            ...
            child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              Text('$score', style: spaceGrotesk(size: 20, weight: FontWeight.w700)),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Financial Health Score', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
          const SizedBox(height: 4),
          Text('Based on leakage, SIP habits & goals',
              style: jakarta(size: 12, color: kMuted)),
          const SizedBox(height: 8),
          ProgressBar(value: score.toDouble(), tone: score >= 60 ? 'positive' : 'warning'),
        ])),
      ]),
    );
  }
      ...
      return Padding(
        ...
        child: Row(children: [
          Container(width: 10, height: 10, decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(3), color: col)),
          const SizedBox(width: 10),
          ...
          const SizedBox(width: 10),
          SizedBox(width: 48, child: Text(_fmtINR(entry.value),
  ...
  Widget _buildSpendSplit(List<Expense> expenses) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Monthly breakdown', style: spaceGrotesk(size: 15, weight: FontWeight.w700)),
      const SizedBox(height: 12),
      ClipRRect(
        borderRadius: BorderRadius.circular(99),
        child: SizedBox(
          ...
          child: Row(children: [
            if (total > 0) ...[
              Flexible(flex: (essential / total * 100).round(), child: ColoredBox(color: kGreen, child: const SizedBox.expand())),
              Flexible(flex: (avoidable / total * 100).round(), child: ColoredBox(color: kAmber, child: const SizedBox.expand())),
              Flexible(flex: (impulse / total * 100).round(), child: ColoredBox(color: kRed, child: const SizedBox.expand())),
            ],
          ]),
        ),
      ),
  ...
  Widget _buildTopLeaks(List<Expense> expenses) {
    final leaks = expenses
        .where((e) => e.tag != 'essential')
        .map((e) => (name: e.name, mo: _monthlyEquiv(e.amount, e.frequency), tag: e.tag))
        .toList()
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Top leaks', style: spaceGrotesk(size: 15, weight: FontWeight.w700)),
      const SizedBox(height: 4),
      Text('Avoidable & impulse expenses ranked by cost',
          style: jakarta(size: 12, color: kMuted)),
      const SizedBox(height: 12),
      ...top.asMap().entries.map((entry) {
        final i = entry.key;
        final l = entry.value;
        final col = l.tag == 'impulse' ? kRed : l.tag == 'avoidable' ? kAmber : kGreen;
        return Padding(
          padding: const EdgeInsets.only(bottom: 10),
          child: Row(children: [
            Container(
              ...
              alignment: Alignment.center,
              child: Text('${i + 1}', style: jakarta(size: 11, weight: FontWeight.w700)),
            ),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(l.name, style: jakarta(size: 13, weight: FontWeight.w600)),
              const SizedBox(height: 4),
              ClipRRect(
                borderRadius: BorderRadius.circular(99),
                child: LinearProgressIndicator(
            ...
            const SizedBox(width: 10),
            Text.rich(TextSpan(children: [
              TextSpan(text: _fmtINR(l.mo),
                  style: spaceGrotesk(size: 13, weight: FontWeight.w700)),
              TextSpan(text: '/mo', style: jakarta(size: 10, color: kMuted)),
            ])),
          ]),
        );
      }),
      ...
      Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('10-year compounded cost', style: jakarta(size: 12, color: kMuted)),
          Text(_fmtINR(_monthlyLeakage(expenses) * 120),
              style: spaceGrotesk(size: 14, weight: FontWeight.w700, color: kRed)),
        ]),
      ),
    ]);
  }
  ...
  Widget _buildNudge(List<Expense> expenses) {
    final leaks = expenses
        .where((e) => e.tag != 'essential')
        .map((e) => (name: e.name, mo: _monthlyEquiv(e.amount, e.frequency)))
        .toList()
      ..sort((a, b) => b.mo.compareTo(a.mo));
    ...
    return GlassCard(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 8),
        Text('Simulate →', style: jakarta(size: 12, color: kTeal, weight: FontWeight.w700)),
      ]),
    );
  }
...
class _StatBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label.toUpperCase(), style: jakarta(size: 9, color: kMuted, letterSpacing: 0.8)),
        const SizedBox(height: 4),
        Text(value, style: spaceGrotesk(size: 16, weight: FontWeight.w700, color: color)),
      ]),
    );
  }
}
...
class _BleedMeta extends StatelessWidget {
  final String label, value;
  final Color color;
  const _BleedMeta({required this.label, required this.value, required this.color});

  @override
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(label, style: jakarta(size: 10, color: kMuted, letterSpacing: 0.5)),
    const SizedBox(height: 2),
    Text(value, style: spaceGrotesk(size: 14, weight: FontWeight.w700, color: color)),
  ]);
}
...
class _ScoreRingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(center, radius,
      Paint()..color = Colors.white.withOpacity(0.06)..style = PaintingStyle.stroke..strokeWidth = 8);
    ...
    canvas.drawArc(
      ...
      -math.pi / 2, sweep, false, arcPaint,
    );
  }
...
class _MiniLineChart extends StatelessWidget {
  final double salary, total, leakage;
  const _MiniLineChart({required this.salary, required this.total, required this.leakage});

  @override
  Widget build(BuildContext context) {
    final months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
    final incomeData = [0.96, 0.98, 0.99, 1.00, 1.01, 1.01].map((f) => salary * f).toList();
    final spendData  = [0.92, 0.95, 0.97, 0.98, 1.01, 1.02].map((f) => total  * f).toList();
    final max = incomeData.reduce(math.max);

    return SizedBox(
      height: 160,
      child: CustomPaint(
        painter: _TwoLinePainter(income: incomeData, spend: spendData, max: max),
        child: Align(
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: months.map((m) => Text(m, style: jakarta(size: 10, color: kMuted))).toList(),
            ),
          ),
        ),
      ),
...
class _TwoLinePainter extends CustomPainter {
  ...
  final double max;
  const _TwoLinePainter({required this.income, required this.spend, required this.max});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawLine(List<double> data, Color color) {
      ...
      canvas.drawPath(path, Paint()..color = color..strokeWidth = 2.5..style = PaintingStyle.stroke..strokeJoin = StrokeJoin.round..strokeCap = StrokeCap.round);
    }
...
class _ExpensesPageState extends State<ExpensesPage> {
  ...
  @override
  void dispose() {
    _nameCtrl.dispose(); _amtCtrl.dispose(); super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return PageFrame(children: [
      ...
      const SizedBox(height: 10),
      Text('Tag every recurring cost once\nand let the app do the rest.',
          style: spaceGrotesk(size: 22, weight: FontWeight.w700, letterSpacing: -0.6)),
      const SizedBox(height: 20),

      // ── Add form ──────────────────────────────────────────────────────────
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Add expense', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
          const Pill(label: 'Quick entry', tone: 'teal'),
        ]),
        const SizedBox(height: 14),
        Row(children: [
          Expanded(child: TextField(controller: _nameCtrl,
            style: jakarta(size: 13), decoration: const InputDecoration(hintText: 'Expense name'))),
          const SizedBox(width: 10),
          Expanded(child: TextField(controller: _amtCtrl, keyboardType: TextInputType.number,
            style: jakarta(size: 13), decoration: const InputDecoration(hintText: 'Amount'))),
        ]),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _DropField(
            value: _freq, onChanged: (v) => setState(() => _freq = v!),
            items: const ['daily', 'weekly', 'monthly'],
          )),
          const SizedBox(width: 10),
          Expanded(child: _DropField(
            value: _tag, onChanged: (v) => setState(() => _tag = v!),
        ...
        SizedBox(
          width: double.infinity,
          child: _PrimaryButton(label: 'Add expense →', onTap: () => _add(state)),
        ),
      ])),
      const SizedBox(height: 12),
      ...
      // ── Burn rate chips ──────────────────────────────────────────────────
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Burn rate', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
        const SizedBox(height: 12),
        MetricRow(items: [
          (label: 'Per day',    value: _fmtINR(totals['leakage']! / 30)),
          (label: 'Per hour',   value: _fmtINR(totals['leakage']! / 30 / 24)),
          (label: 'Per minute', value: _fmtINR(totals['leakage']! / 30 / 24 / 60)),
        ]),
      ])),
      const SizedBox(height: 12),
      ...
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Expense list', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
          const Pill(label: 'Tap tag to cycle', tone: 'teal'),
        ]),
        const SizedBox(height: 12),
        ...state.expenses.map((e) => _ExpenseRow(
          expense: e,
...
class _ExpenseRow extends StatelessWidget {
  ...
  const _ExpenseRow({required this.expense, required this.onToggle, required this.onDelete});

  @override
  Widget build(BuildContext context) {
    final col = expense.tag == 'essential' ? kGreen : expense.tag == 'avoidable' ? kAmber : kRed;
      ...
      child: Row(children: [
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Text(expense.tag, style: jakarta(size: 10, color: col, weight: FontWeight.w700)),
          ),
        ),
        const SizedBox(width: 8),
        GestureDetector(
...
class _SimulatorPageState extends State<SimulatorPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final later = _futureValue(state.sipAmount, state.sipReturn, state.sipMonths - 6);
    final lost  = now - later;

    return PageFrame(children: [
      const Pill(label: 'Simulator', tone: 'positive'),
      const SizedBox(height: 10),
      Text('Compare start today vs waiting,\nthen redirect leaks into SIPs.',
          style: spaceGrotesk(size: 22, weight: FontWeight.w700, letterSpacing: -0.6)),
      const SizedBox(height: 4),
      Text("The corpus drops hard when you delay. That's the whole pitch.",
      ...
      GlassCard(child: Column(children: [
        _SliderRow(
          ...
          slider: Slider(
            value: state.sipAmount, min: 1000, max: 50000, divisions: 98,
        ...
        _SliderRow(
          ...
          slider: Theme(
            data: Theme.of(context).copyWith(
              sliderTheme: SliderThemeData(activeTrackColor: kBlue, thumbColor: kBlue)),
            child: Slider(
              value: state.sipReturn, min: 6, max: 18, divisions: 24,
        ...
        _SliderRow(
          ...
          slider: Slider(
            value: state.sipMonths.toDouble(), min: 12, max: 240, divisions: 19,
      ...
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Start today vs later', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
          const Pill(label: 'Compounding gap', tone: 'warning'),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _CompareBox(label: 'Start now',              value: _fmtINR(now),   color: kTeal)),
          const SizedBox(width: 10),
          Expanded(child: _CompareBox(label: 'Start 6 months later', value: _fmtINR(later), color: kAmber)),
        ]),
        const SizedBox(height: 14),
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Lost by waiting 6 months', style: jakarta(size: 11, color: kMuted)),
              Text(_fmtINR(lost), style: spaceGrotesk(size: 22, weight: FontWeight.w700, color: kRed)),
            ])),
          ]),
        ),
        const SizedBox(height: 16),
        // Chart
        Text('Corpus growth projection', style: jakarta(size: 12, color: kMuted)),
        const SizedBox(height: 10),
        _SipChart(sip: state.sipAmount, rate: state.sipReturn, months: state.sipMonths),
      ])),
      const SizedBox(height: 12),

      // ── Redirect leak ─────────────────────────────────────────────────────
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Redirect a leak', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
        const SizedBox(height: 8),
        Text('Every avoidable expense can be redirected into your SIP.',
            style: jakarta(size: 12, color: kMuted)),
        const SizedBox(height: 14),
        ...state.expenses.where((e) => e.tag != 'essential').take(4).map((e) {
          final m = _monthlyEquiv(e.amount, e.frequency);
          ...
          return Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(e.name, style: jakarta(size: 13, weight: FontWeight.w600)),
              Text.rich(TextSpan(children: [
                TextSpan(text: '→ +${_fmtINR(bonus)}',
                    style: spaceGrotesk(size: 12, weight: FontWeight.w700, color: kTeal)),
              ])),
            ]),
          );
        }),
...
class _CompareBox extends StatelessWidget {
  final String label, value;
  final Color color;
  const _CompareBox({required this.label, required this.value, required this.color});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: jakarta(size: 11, color: kMuted)),
      const SizedBox(height: 4),
      Text(value, style: spaceGrotesk(size: 17, weight: FontWeight.w700, color: color)),
    ]),
  );
}
...
class _SipChart extends StatelessWidget {
  ...
  final int months;
  const _SipChart({required this.sip, required this.rate, required this.months});
...
class _GoalsPageState extends State<GoalsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return PageFrame(children: [
      ...
      const SizedBox(height: 10),
      Text('Turn target dates\ninto a monthly number.',
          style: spaceGrotesk(size: 22, weight: FontWeight.w700, letterSpacing: -0.6)),
      const SizedBox(height: 4),
      Text('Each goal shows progress, required monthly saving, and invest vs save comparison.',
          style: jakarta(size: 13, color: kMuted)),
      const SizedBox(height: 20),
      ...
      ...state.goals.map((g) {
        ...
        final needed = _monthlySavingsRequired(g.targetAmount, g.savedAmount, g.targetDate);
          ...
          child: GlassCard(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                Pill(label: 'P${g.priority}', tone: g.priority == 1 ? 'warning' : 'positive'),
              ]),
              const SizedBox(height: 12),
              ProgressBar(value: pct * 100, tone: pct >= 0.5 ? 'positive' : 'warning'),
              const SizedBox(height: 10),
              MetricRow(items: [
              ...
              Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('Invest same amount → 2-year corpus',
                      style: jakarta(size: 11, color: kMuted)),
                  Text(_fmtINR(investResult),
                      style: spaceGrotesk(size: 13, weight: FontWeight.w700, color: kBlue)),
                ]),
              ),
            ]),
          ),
      ...
      // ── Suggested cuts ──────────────────────────────────────────────────
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Suggested cuts', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
        const SizedBox(height: 10),
        ...['Trim one delivery habit', 'Remove duplicate subscriptions', 'Redirect one impulse purchase']
            .map((s) => Padding(
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(_showForm ? Icons.expand_less : Icons.add_rounded, color: kTeal, size: 18),
            const SizedBox(width: 8),
            Text(_showForm ? 'Cancel' : 'Add new goal',
                style: jakarta(size: 13, weight: FontWeight.w700, color: kTeal)),
          ]),
        ),
      ),
      if (_showForm) ...[
        const SizedBox(height: 10),
        GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('New goal', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
          const SizedBox(height: 12),
          TextField(controller: _nameCtrl, style: jakarta(size: 13),
...
class InsightsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final state = AppStateProvider.of(context);
    final leakage = _monthlyLeakage(state.expenses);
    final personality = _detectPersonality(leakage, state.monthlySalary, state.sipAmount);

    final badges = [
      (name: 'Leak Finder',    hint: 'Tagged 3 expenses correctly', unlocked: true),
      (name: 'SIP Starter',    hint: 'Started a monthly SIP',       unlocked: true),
      (name: 'No-Spend Week',  hint: '7 days under avoidable spend', unlocked: false),
    ...
    return PageFrame(children: [
      ...
      const SizedBox(height: 10),
      Text('Package habits into a personality,\nthen show the rupee consequence.',
          style: spaceGrotesk(size: 22, weight: FontWeight.w700, letterSpacing: -0.6)),
      const SizedBox(height: 20),
      ...
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Spending personality', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
      ...
      Row(children: [
        Expanded(child: _InsightTile(
          label: '₹ saved from leaks', value: _fmtINR(leakage * 8),
          note: 'annualized', color: kTeal,
        )),
        const SizedBox(width: 10),
        Expanded(child: _InsightTile(
          label: '10-year corpus', value: _fmtINR(_futureValue(state.sipAmount, 12, 120)),
          note: 'from SIP', color: kBlue,
        )),
      ]),
      const SizedBox(height: 10),
      Row(children: [
        Expanded(child: _InsightTile(
          label: 'Health score', value: _healthScoreLabel(state),
          note: personality, color: kGreen,
        )),
        const SizedBox(width: 10),
        Expanded(child: _InsightTile(
          ...
          value: state.monthlySalary > 0
              ...
              : '0%',
          note: 'of monthly outflow', color: kAmber,
        )),
      ]),
      const SizedBox(height: 12),
      ...
      // ── Recommendations ──────────────────────────────────────────────────
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Recommendations', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
        const SizedBox(height: 12),
        ...recs.map((r) => Padding(
          padding: const EdgeInsets.only(bottom: 12),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Expanded(child: Text(r.label, style: jakarta(size: 13, weight: FontWeight.w600))),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(99),
              ...
              child: Text(_fmtINR(r.saving),
                  style: jakarta(size: 11, color: kTeal, weight: FontWeight.w700)),
            ),
          ]),
        )),
      ])),
      ...
      // ── Badges wall ──────────────────────────────────────────────────────
      GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Badges wall', style: spaceGrotesk(size: 14, weight: FontWeight.w700)),
        const SizedBox(height: 12),
        ...badges.map((b) => Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(16),
            color: b.unlocked ? kTeal.withOpacity(0.06) : Colors.white.withOpacity(0.02),
            border: Border.all(
              color: b.unlocked ? kTeal.withOpacity(0.24) : Colors.white.withOpacity(0.05)),
          ),
          child: Row(children: [
            Icon(b.unlocked ? Icons.check_circle_rounded : Icons.lock_rounded,
                color: b.unlocked ? kTeal : kMuted, size: 18),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(b.name,
                  style: jakarta(size: 13, weight: FontWeight.w700,
                      color: b.unlocked ? kText : kMuted)),
              Text(b.hint, style: jakarta(size: 11, color: kMuted)),
            ])),
          ]),
        )),
      ])),
...
class _InsightTile extends StatelessWidget {
  ...
  final Color color;
  const _InsightTile({required this.label, required this.value, required this.note, required this.color});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: jakarta(size: 11, color: kMuted)),
      const SizedBox(height: 6),
      Text(value, style: spaceGrotesk(size: 18, weight: FontWeight.w700, color: color)),
      const SizedBox(height: 2),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(99),
          gradient: const LinearGradient(colors: [kTeal, kGreen]),
          boxShadow: [BoxShadow(color: kTeal.withOpacity(0.22), blurRadius: 24)],
        ),
        alignment: Alignment.center,
        child: Text(label,
...
String _catOf(String name) {
  ...
  if (n.contains('swiggy') || n.contains('zomato') || n.contains('cafe')) return 'Food & Delivery';
  if (n.contains('netflix') || n.contains('ott') || n.contains('prime') || n.contains('spotify')) return 'Subscriptions';
  if (n.contains('metro') || n.contains('uber') || n.contains('ola')) return 'Transport';
...
Color _catColor(String cat) {
  switch (cat) {
    case 'Food & Delivery':  return kRed;
    case 'Subscriptions':    return kAmber;
    case 'Transport':        return kBlue;
    case 'Digital & Cloud':  return const Color(0xFFA78BFA);
    case 'Fitness & Health': return kGreen;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // Title
        Text('Enable Behavior Monitor',
            style: spaceGrotesk(size: 20, weight: FontWeight.w800)),
        const SizedBox(height: 6),
        Text('Expense Autopsy needs two permissions to watch for spending traps in real time.',
            style: jakarta(size: 13, color: kMuted)),
        ...
        SizedBox(
          ...
          child: GestureDetector(
            onTap: () async {
              ...
              if (!hasOverlay) await MonitorService.requestOverlayPermission();
            ...
            child: Container(
              ...
              child: Text('Grant permissions →',
                  style: jakarta(size: 15, weight: FontWeight.w700,
                      color: const Color(0xFF061015))),
            ),
          ),
        ),
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        color: granted
            ...
            : kPanel,
        border: Border.all(
            color: granted ? kGreen.withOpacity(0.2) : kBorder),
      ),
      child: Row(children: [
        Container(
